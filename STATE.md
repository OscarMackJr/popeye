...
- [Popeye integration roadmap](./specs/ROADMAP_integration_popeye.md) for the working scope/MVP boundary between Popeye, hometown, and program-level POCs.
...
- `config/litellm.azure.example.yaml`: per-region gateway config with fallback-chain and no-prompt-logging guardrails carried from the POC.
